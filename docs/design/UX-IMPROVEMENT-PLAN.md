...
不具合修正（GitHub issues #122–#128）とは独立した、UX設計変更のロードマップ。
...
## Wave 1 — 「画面が消えない・操作に必ず返事が来る」（BE変更ゼロ）

| # | 内容 | 状態 |
|---|------|------|
| 1 | `(shell)`/`(authed)` route group + `AuthGate`。14ページの複製ガードを削除、シェルが遷移をまたいで生存 | PR-A |
| 2 | グローバル `SWRConfig`（keepPreviousData）— タブ/フィルタ切替の空白点滅解消 | PR-A |
| 3 | `RewardModalHost` をシェルへ — どの入口からでも報酬儀式が発生（/billing/return は抑止） | PR-A |
| 4 | returnTo（`/login?returnTo=` + open-redirect検証 + OAuth往復は sessionStorage） | PR-A |
| 5 | サイドバー再編 14→11 + HERO_STATUSメニュー: badges→NAVIGATION（名声）、shop 3→1（共有 `ShopTabs`）、mints/mypage→フッターメニュー、`~/@{username}` 復活（正典 home.html にあり実装脱落）。**summon は正典どおり2項目維持**（summon.html の「PRESTIGE ZONE — 課金導線なし」刻印に琥珀タブを持ち込まない） | PR-B |
| 6 | `mobileFullWidth` 廃止 + <768px 全ページ統一（サイドバー非表示）+ ハンバーガー→`MobileNavDrawer`（navConfig/HERO_STATUS 再利用、フォーカストラップ、遷移時自動クローズ） | PR-B |
| 7 | `InlineActionResult`（items の成功ブロックを汎用化）→ スキン購入/装備・鋳造・強化の操作直近に確定表示 | PR-C |
| 8 | `BalanceShortfall`（**コイン/ルーン非対称**: コイン不足=「PRをマージして集めましょう」購入導線なし / ルーン不足=`/shop/runes` リンク）→ mints・forge へ適用 | PR-C |
| 9 | 召喚結果モーダルに `[ 図鑑で確認 → ]` フッター（通常/限定とも） | PR-C |
| 10 | 変異成功時に wallet / dex を即 `mutate`（shop の既存パターンを召喚/レベルアップ等へ展開。issue #125 対応） | PR-C |
...
- 通常召喚への `isNew` 延伸（限定は実装済み。契約のコピーで済む小さな依頼）
