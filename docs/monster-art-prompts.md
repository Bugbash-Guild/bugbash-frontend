...
- 単体生成前に、系統ごとの `background_key_color` を必ず1色決める。必要なら予備キー色も1色決める。
- プロンプトには「完全な単色背景」「影なし」「床なし」「そのキー色をモンスター内で使わない」を明記する。
- 迷ったら彩度の高い人工色を使うが、キャラの発光色・アクセント色・ラベル色と近い色は避ける。
- 緑・シアン・マゼンタを機械的に固定しない。系統の色に応じて、黄緑、蛍光オレンジ、純赤、純青、マゼンタなどから衝突しない色を選ぶ。
...
- 背景を抜くときは、画像全体からキー色を一括削除する。キー色は本体側に使っていない前提で選ぶため、外周につながっていない内側の穴や囲まれた背景も必ず抜く。
- 外周につながっている領域だけを透過する方式は使わない。内側に囲まれた背景が残るため、ゲーム用アセットの切り抜き手段として採用しない。
- キー色が本体・発光・半透明素材・ラベル・縁取りに混ざった疑いがある場合は、その切り抜き結果を使わず、別のキー色で再生成する。
