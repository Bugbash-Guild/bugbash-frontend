...
- **unified**: ^11.0.5（テキスト処理パイプライン）
- **remark-gfm**: ^4.0.1（GitHub Flavored Markdown）
- **rehype-katex**: ^7.0.1（数式レンダリング）
- **rehype-sanitize**: ^6.0.0（XSS対策）
- **mermaid**: ^11.12.0（ダイアグラム描画）
...
# localhost:8080からapi.jsonを取得し、src/lib/apiClient/types/schema.d.tsを生成
npm run schema

# APIモックサーバー起動（ポート18081）
npm run mock
```
...
# Claude Code用ドキュメント生成
npm run generate:claudedocs

# パッケージパッチ適用（postinstall時に自動実行）
npm run postinstall
```
...
- `noFallthroughCasesInSwitch: true`（switch文のfallthrough防止）
- `noUncheckedSideEffectImports: true`（副作用インポートのチェック）
- `target: "ES2023"`（最新のECMAScript機能使用）

#### 型インポートの強制

- `@typescript-eslint/consistent-type-imports`により、型インポートは必ず`import type`または`import { type Foo }`形式を使用
- 例: `import { type ComponentType } from 'react'`

### ESLintルール

#### 必須ルール

1. **インポートの自動整理**（`simple-import-sort`）

    - インポートは自動的にアルファベット順にソート
    - グループ分けは自動処理

2. **未使用importの削除**（`unused-imports`）

    - 未使用のimportは自動削除
    - 未使用変数は警告（`_`プレフィックスで無視可能）

3. **React Hooks Rules**（`react-hooks`）

    - フックの依存配列を厳密にチェック

4. **アクセシビリティ**（`jsx-a11y`）

    - WCAG準拠のアクセシビリティルールを適用

5. **console制限**

    - `console.log`は警告（`console.warn`と`console.error`のみ許可）

6. **制限付きインポート**
    - `wouter`の`Link`コンポーネント直接使用禁止
    - 代わりに`src/components/Link`を使用
...
- **Tailwindクラスの自動整理**: `prettier-plugin-tailwindcss`により、クラス名が推奨順序に自動整理
...
ESLintにより自動的に以下の順序に整理されます：

1. Reactなどのコアライブラリ
2. サードパーティライブラリ
3. パスエイリアスを使用したローカルインポート（`~`プレフィックス）
4. 相対パスでのローカルインポート
...
animation: {
  ...
  'kawaii-spin': 'spin 1.2s cubic-bezier(0.68, -0.2, 0.32, 1.2) infinite',
}
```
...
ページやコンポーネントと**同じディレクトリ**に配置：

```
src/routes/settings/
├── index.tsx
└── settings.spec.ts  ← 同じ場所
```
...
- **ポート**: 13000（開発サーバー自動起動）
- **ブラウザ**: Chromium、Firefox、WebKit
- **APIモック**: デフォルトで`http://localhost:18081`（Prism Mock Server）
- **タイムアウト**: 20秒
- **リトライ**: CI環境では2回
...
```bash
npm run mock  # Prism Mock Server起動（ポート18081）
```
...
#### バンドル分割戦略（manualChunks）

```typescript
manualChunks: (id) => {
  // Reactコア
  if (id.includes('react') || id.includes('wouter')) return 'core';
  ...
  if (id.includes('mermaid')) {
    if (id.includes('flowDiagram')) return 'mermaid-flow';
    if (id.includes('sequenceDiagram')) return 'mermaid-sequence';
    // ...
  }
...
assetFileNames: (assetInfo) => {
  ...
  // フォント（ハッシュなし、キャッシュ活用）
  if (/\.(woff2?|ttf|otf)$/.test(fileName)) {
...
```json
{
  ...
  "rewrites": [
    {
      "source": "/tasks/:id",
      "destination": "/api/inject-head" // OGP注入API
    },
    {
      "source": "/((?!@.*)(?!.*\\..*).*)",
...
- `VITE_PAYMENT_ACTIVE_ORGANIZATION_ID`: 決済組織ID
- `PORT`: 開発サーバーポート（デフォルト: 3000）
- `DISABLE_PWA`: PWA無効化フラグ（Storybook用）
...
```bash
npm run build
# → rollup-plugin-visualizer によりstats.htmlが生成される
```

`stats.html`を開いてバンドルサイズを視覚的に確認。
...
→ `npm run schema`でAPI型を再生成

#### ビルドエラー

→ `npm run lint`でESLintエラーを確認・修正

#### テストエラー

→ `npm run test:e2e`でMock Serverが起動しているか確認
...
Dialogは複数の種類を用途に応じて使い分けます。

```typescript
import { useDialog } from '~lib/dialog';
import { AlertDialog, ConfirmDialog, PromptDialog, CustomDialog } from '~components/Dialog';
...
**問題**: `npm run preview`後、`npm run dev`が古いコードを表示

**解決**:

1. Chrome DevTools → Application タブ
2. Service Workers → "Unregister" をクリック
3. ページをリロード

**注意**: スーパーリロード（Cmd+Shift+R）では解決しません。

---

### 2. usePromiseのキャッシュが更新されない

**問題**: APIで更新したのにUIに反映されない

**解決**:

```typescript
// データ更新後に必ず実行
await apiClient.PUT('/api/users/me', { body: data });
mutateAllPromise(); // 全usePromiseを再検証
```

**注意**: `clearPromiseSoftCache()`だけでは再取得されません。

---

### 3. 401リダイレクトが動作しない

**問題**: ログインが必要なページで401が返るが、/authにリダイレクトされない

**原因**: `useCustomUnAuthorizedInterceptor`が別のコンポーネントで使用されている

**解決**: インターセプターを削除するか、該当コンポーネントをアンマウント

---

### 4. Tailwindクラスが競合する

**問題**: `className="p-4 p-2"`で両方のpaddingが適用される

**解決**: `cn`関数を使用

```typescript
// ✅ Good: cn関数が自動的に競合を解決（p-2のみ適用）
className={cn('p-4', condition && 'p-2')}

// ❌ Bad: 文字列結合（両方適用される）
className={`p-4 ${condition ? 'p-2' : ''}`}
```
...
**問題**: SVGファイルの色が固定されている

**解決**: `text-*`クラスで色を制御（currentColor変換済み）

```typescript
import Icon from './icon.svg?react';

// ✅ Good
<Icon className="text-blue-500" />  // 青色
...
**問題**: Playwrightテストが20秒でタイムアウト

**原因**: Mock Serverが起動していない、または重い処理

**解決**:

```bash
# Mock Server起動を確認
npm run mock  # ポート18081で起動

# または、タイムアウトを延長（playwright.config.ts）
timeout: 30000  // 30秒
```

---

### 7. 型エラー: Cannot find module 'schema.d.ts'
...
型安全にCSVファイルをエクスポートできます。

```typescript
import { createHeaders, exportToCSV } from '~lib/csv';

// データ
const users = [
  { id: 1, name: '田中太郎', age: 30, email: 'tanaka@example.com' },
  { id: 2, name: '山田花子', age: 25, email: 'yamada@example.com' },
];

// ヘッダー定義（型推論が効く）
const headers = createHeaders<(typeof users)[number]>([
  { key: 'id', label: 'ID' },
  { key: 'name', label: '名前' },
  { key: 'age', label: '年齢' },
...
exportToCSV(users, headers, 'users', {
  ...
  includeHeaders: false, // ヘッダー行を含めない
  encoding: 'Shift_JIS', // エンコーディング（デフォルト: UTF-8）
});
```
...
.dots-loading::after {
  content: '...';
  animation: dots-animation 0.6s steps(4) infinite;
}
...
/* Tailwindユーティリティ */
.h-header-nav {
  height: var(--header-nav-height); /* 66px */
}
...
.w-sidebar {
  width: var(--sidebar-width); /* 220px / 0px(SP) */
}
...
1. **SPブレークポイント**: `640px`ではなく`768px`を使用
2. **cn関数**: クラス名結合には必ず使用（Tailwindの競合解決）
3. **createTitle**: ページタイトルには必ず使用
4. **useBodyCustomCSS**: body要素のクラス変更に使用
5. **CSV出力**: `createHeaders` + `exportToCSV`で型安全に
6. **ファイルバリデーション**: `validateFile`で統一
7. **URL判定**: `isUrl`, `isWebUrl`, `isExternalUrl`を使い分け
8. **Cookie取得**: `cookie.get()`を使用
9. **日付フォーマット**: `dateUtil`のユーティリティを活用
10. **インタラクティブ**: `.interactive`クラスでクリック時アニメーション
11. **CSS変数**: `--header-nav-height`等のカスタムプロパティを活用
12. **Helmet**: ページタイトル・metaタグの管理に使用
13. **定数管理**: グローバルは`constants/`、ローカルは同じディレクトリ
14. **セキュリティ**: `Markdown`コンポーネント、`apiClient`で自動対策
